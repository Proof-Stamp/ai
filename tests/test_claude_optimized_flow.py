...
class ClaudeOptimizedFlowTests(unittest.TestCase):
    ...
    def test_finalizer_rejects_ai_generated_complete_without_evidence_reference(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("use 'unknown' when completeness cannot be established", result.stderr)
            self.assertFalse(
